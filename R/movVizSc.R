#' @importFrom grid unit grid.newpage pushViewport gpar
#' plotViewport viewport grid.layout popViewport grid.xaxis
#' grid.text grid.rect arrow grid.raster grid.lines
NULL
...
#' @param scPACds a PACdataset storing single cell PACs. The PACds@anno should have columns chr/strand/coord.
...
runDiffusionMap<- function(mat){
  e <- try({
    dif <- destiny::DiffusionMap(destiny::as.ExpressionSet(as.data.frame(mat)));
...
plotHeatmapSC <- function(mat, breaks, pal, fixed_max, PAwidth, txdb, scPACds, gene){
  ...
  if(is.null(txdb)){
    ...
    dt_gtf %>% data.table::setkey(transcript_id)
...
gtfToDtExon <- function(txdb = txdbmm10){
  ...
  data.table::as.data.table(dt_gtf[,c("seqnames", "start", "end", "strand", "type", "gene_id", "transcript_id")])
}
